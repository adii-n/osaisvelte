...
export let use = [], element = undefined, className = '', override = {}, 
// daysRefs: $$MonthProps['daysRefs'] = undefined,
disableOutsideEvents = false, month, locale = 'en', value = undefined, range = undefined, weekdayLabelFormat = undefined, minDate = undefined, maxDate = undefined, hideWeekdays = false, size = 'sm', fullWidth = false, preventFocus = false, focusable = true, firstDayOfWeek = 'monday', hideOutsideDates = false, onChange = (value) => undefined, onDayMouseEnter = undefined, onDayKeyDown = undefined, renderDay = undefined, 
// dayStyle: $$MonthProps['dayStyle'] = undefined,
dayClassName = undefined, excludeDate = undefined, isDateInRange = no, isDateFirstInRange = no, isDateLastInRange = no;
export { className as class };
/** An action that forwards inner dom node events from parent component */
const forwardEvents = createEventForwarder(get_current_component());
const castType = (value) => value;
const days = getMonthDays(month, firstDayOfWeek);
const hasValue = Array.isArray(value)
    ? value.every((item) => item instanceof Date)
    : value instanceof Date;
const hasValueInMonthRange = value instanceof Date &&
